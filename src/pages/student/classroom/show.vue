            ...
            <div class="class_topic_warp" v-for="(item,index) in showTopicList" :key="index">
                ...
                <div class="subjective_warp" v-if="item.quesetionType ==='subjective'">
                    ...
                    <div class="button_warp" v-if="item.questionStatus !=='FINISH'">
                        <div class="subjective_submit" v-show="!objectiveAnswer[index].answer"
                             @click="answerQuestions(item.questionId)">开始答题
                        </div>
                        <div class="subjective_submit" v-show="objectiveAnswer[index].answer"
                             @click="modifyAnswer(item.questionId)">修改答案
                        </div>
                    </div>
                    <board :subjectiveAnswer="objectiveAnswer" :id="item.questionId"
                           v-show="isAnswer(objectiveAnswer[index].answer)"/>
                    <!--老师关闭回答-->
                    <div class="closeReply" v-if="item.questionStatus === 'FINISH'">
                        <span class="text">老师已终止{{item.title}}的作答</span>
                    </div>
    ...
    export default {
            ...
            boardImg(val) {
                ...
                sendSubjectPicByString(val.data)
                    .then(res => {
                        ...
                        //提交主观题答案
                        sendSubjectiveAnswer(this.course, this.subjectiveId, 123, 'test', res.data.data.stubForSubjective)
                            .then(res => {
                                console.log(res);
                                Toast({
                                    message: '答案提交成功！',
        ...
        methods: {
            ...
            answerQuestions(id) {
                ...
                window.HandwrittenBoard.isConnect(function (res) {
                    console.log(res);
                }, function (res) {
                    console.log(res);
                ...
                for (let i = 0; i < this.objectiveAnswer.length; i++) {
                    ...
                    if (this.objectiveAnswer[i].id === id) {
                        ...
                        if (this.reply.length === 0) {
                            this.reply.push({id: id, answer: [this.objectiveAnswer[i].answer]});
                        ...
                        for (let j = 0; j < this.reply.length; j++) {
                            ...
                            if (this.reply[j].id === id) {
                                if (this.reply[j].answer[this.reply[j].answer.length - 1] === this.objectiveAnswer[i].answer) {
                                    return false;
                                } else {
                    ...
                    if (t.answerForStudent) {
                        if (t.quesetionType === "objective") {
                            ...
                            if (t.answerForStudent || t.answerForTeacher) {
                                let img = await getSubjectPic(t.answerForTeacher ? t.answerForTeacher.stubForSubjective : t.answerForStudent.stubForSubjective);
            ...
            connect() {
                ...
                this.stompClient.connect(headers, function (frame) {
                    ...
                    _this.subscription = _this.stompClient.subscribe('/topic/course/student/' + _this.course, function (response) {
                ...
                .objective_warp {
                    ...
                    .submit_warp {
                        ...
                        .objective_submit:active {
                            box-shadow: 10px 40px 10px rgba(0, 17, 27, .1) inset;
                        }
                    }
                    .closeReply {
                        margin-top: 2.5rem;
