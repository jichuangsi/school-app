            ...
            <div class="subjective_warp">
                ...
                <div class="button_warp">
                    <div class="subjective_submit"
                         @click="modifyAnswer(subjectiveMsg.questionId)">批改答案
                    </div>
                </div>
...
<script>
    ...
    import {getQuestion, getSubjectPic, sendAnswer, sendSubjectPicByString} from '@/api/teacher/classroom'
    ...
    export default {
        ...
        data() {
            return {
                ...
                subjectiveTopic: [{    //页面内容
                    id: 1,
                    type: 'subjective',
                    title: '主观题-1',
                    content: '有两根铁丝，第一根用去 米，第二根用去 ，剩下的一样长，两根铁丝和原来相比如何。'
                }],
                scores: '',           //分数
        ...
        methods: {
            ...
            async getImg() {
                ...
                if (self.subjectiveMsg.answerForTeacher) {
                    stubForSubjective = self.subjectiveMsg.answerForTeacher[self.subjectiveMsg.answerForTeacher.findIndex(x => {
                        return x.studentId == self.studentId
                    })];
                } else {
                    stubForSubjective = self.subjectiveMsg.answerForStudent[self.subjectiveMsg.answerForStudent.findIndex(x => {
                        return x.studentId == self.studentId
                    })];
                }
                try {
            ...
            async submited() {
                let self = this;
                try {
                    await sendAnswer(self.topicId, self.answerId, null, self.scores, self.picForSubjective, self.teacherId, self.teacherName);
                    Toast({
                        message: '提交成功',
                        position: 'bottom'
                    });
